...
class hashieclean(plugins.Plugin):
    __author__ = 'Artur Oliveira, Aaron Cupp (IamMrCupp)'
    __version__ = '2.0.0'
    __license__ = 'GPL3'
    __description__ = '''

Version 2.0.0 of hashieclean plugin for Pwnagotchi. 
This has been refactored to be python 3.11 compatible, and to improve performance.

This plugin removes "lonely pcaps", those can't be converted
...
Changelog v2.0.0:
    ...
    - Fixed security issues with shell command execution
    - Better path handling using pathlib
    - Added validation for required tools
                        '''
    ...
    def _validate_dependencies(self):
        ...
        try:
            ...
            if result.returncode != 0:
                logging.warning("[hashieclean] hcxpcapngtool not found. Install it for full functionality.")
        except Exception as e:
            logging.error(f"[hashieclean] Error checking dependencies: {e}")

    def on_config_changed(self, config):
        """Called when config changes and main loop is about to start."""
        try:
            ...
            logging.error(f"[hashieclean] Config error: {e}")
        except Exception as e:
            logging.error(f"[hashieclean] Unexpected error in on_config_changed: {e}")
